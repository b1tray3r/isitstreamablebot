...
func TestExtractSpotifyLinksFromText(t *testing.T) {
	tests := []struct {
		name     string
		text     string
		expected []string
	}{
		{
			...
			text: "Check out this song: https://open.spotify.com/track/0WVgnZcUUvQVoaa2gEnv3Q?si=5e37c0a62f894f84",
			expected: []string{
		...
		},
		{
			...
			expected: []string{
				"https://open.spotify.com/track/0WVgnZcUUvQVoaa2gEnv3Q?si=5e37c0a62f894f84",
				"https://open.spotify.com/track/1a2b3c4d5e6f7g8h9i0jklmnopqrstuvwx?si=1234567890abcdef",
			},
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result := ExtractSpotifyLinksFromText(tt.text)
